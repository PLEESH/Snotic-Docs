...
- install and operate one instance of this release on one server the recipient
  owns or controls;
- use that instance for the recipient's own operations and to manage WordPress
  sites the recipient is authorized to host; and
- make reasonable backup and disaster-recovery copies of the supplied binary
  distribution.

There is no software charge, subscription activation, or seven-day software
expiry for this release. This grant does not include Snotic Managed, server
operation by Pleesh, managed backups, infrastructure, an SLA, or support unless
those services are agreed separately.
